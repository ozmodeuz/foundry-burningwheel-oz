export class BWSettingSheet extends ActorSheet {
    ...
    getData() {
        return {
            ...
            lifepaths: Array.from(this.actor.items.values()).map(i => i.data).sort((a, b) => a.data.order - b.data.order),
    ...
    async insertItemAtIndex(event, index) {
        ...
        if (dragData.actorId === this.actor.id) {
            ...
            if (dragData.data) {
                ...
                itemData = (await game.packs?.find(p => p.collection === dragData.pack).getDocument(dragData.id || ""))?.data;
            }
            else if (dragData.actorId) {
                itemData = (game.actors?.find((a) => a.id === dragData.actorId))?.items.get(dragData.id || "")?.data;
            }
            else {
                itemData = game.items?.find((i) => i.id === dragData.id)?.data;
            }
